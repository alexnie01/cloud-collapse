...
def status_colors(escaped: np.ndarray, collided: np.ndarray, kinetic_energy: np.ndarray) -> np.ndarray:
    """Per-particle RGB, straight from stored/derived per-frame data (never recomputed elsewhere):
    green = escaped, red = collided sometime since the last recorded frame, otherwise a
    white-to-red gradient by kinetic energy (redder = faster), normalized against the
    fastest currently-bound, non-colliding particle in this frame.
    """
    n = len(escaped)
    colors = np.full((n, 3), 255, dtype=np.uint8)

    bound = ~escaped & ~collided
    if bound.any():
    ...
    colors[collided & ~escaped] = _RED
    colors[escaped] = _GREEN
    return colors
...
def animate(store_path: str, fps: int = 30, export: str | None = None) -> None:
    ...
    positions0, velocities0, escaped0, collided0 = read_frame(root, 0)
    cloud = pv.PolyData(positions0)
    cloud["colors"] = status_colors(escaped0, collided0, per_particle_ke(velocities0))
    ...
    plotter.add_text(
        "white->red = kinetic energy   red = collided   green = escaped",
    ...
    def render_frame(frame_idx: int, paused: bool) -> None:
        positions, velocities, escaped, collided = read_frame(root, frame_idx)
        cloud.points = positions
        cloud["colors"] = status_colors(escaped, collided, per_particle_ke(velocities))
...
def matplotlib_fallback(store_path: str) -> None:
    ...
    scatter = ax.scatter(
        ...
        positions0[:, 2],
        s=2,
        c=status_colors(escaped0, collided0, per_particle_ke(velocities0)) / 255.0,
    )
    ...
    def update(frame_idx: int):
        ...
        scatter.set_color(status_colors(escaped, collided, per_particle_ke(velocities)) / 255.0)
...
def plot_diagnostics(store_path: str) -> None:
    ...
    frame cadence -- hence the two different time axes below.
    """
    root = open_store(store_path)
